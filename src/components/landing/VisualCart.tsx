...
const VisualCart: React.FC<VisualCartProps> = ({ 
  ...
  return (
    <AnimatePresence>
      <motion.div 
        className="visual-cart-container"
        initial={{ y: 100, opacity: 0, scale: 0.9 }}
        animate={{ y: 0, opacity: 1, scale: 1 }}
        exit={{ y: 100, opacity: 0, scale: 0.9 }}
        transition={{ type: "spring", damping: 25, stiffness: 200 }}
      >
        <div className="visual-cart-header">
          <div className="header-info">
            <h3>Tu Arquitectura</h3>
            <span className="pack-name-small">
              {selectedPack ? selectedPack.nombre : 'Personalizada'}
            </span>
          </div>
          <button className="remove-cart-btn" onClick={onRemove} title="Vaciar Carrito">
            <X size={18} />
          </button>
        </div>
