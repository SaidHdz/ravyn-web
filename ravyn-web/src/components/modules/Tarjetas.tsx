            ...
            <div id="stack-container" style={{ position: 'relative', width: '300px', height: '450px', margin: '0 auto' }}>
              {cartasRestantes.map((carta, index) => {
                ...
                const style: React.CSSProperties = {
                  ...
                  width: '100%',
                  height: '100%',
                  transition: isDragging ? 'none' : 'transform 0.4s ease-out, opacity 0.3s ease',
                  transform: isTop 
                    ? `translateX(${dragOffset.x}px) rotate(${rotation}deg)` 
                    : `scale(${0.9 + (index / cartasRestantes.length) * 0.1}) translateY(${(cartasRestantes.length - 1 - index) * -8}px)`,
                  opacity: isTop ? 1 : 0.95, // Mantener visibilidad alta para el stack
                  touchAction: 'none',
                  visibility: 'visible'
                };
